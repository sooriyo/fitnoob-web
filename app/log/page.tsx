...
export default function LogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchEntries();
      alert("Metrics committed successfully!");
    } catch (err: any) {
  ...
  const markWorkoutDone = async () => {
    ...
    try {
      ...
      await fetchEntries();
      alert("Protocol completed for the specified session date.");
    } catch (err: any) {
      alert("Error: " + err.message);
    } finally {
      setSaving(false);
  ...
  return (
    <div className="space-y-12 pb-32 animate-in fade-in duration-700">
      ...
      {/* 01. PERFORMANCE PROTOCOL */}
      <section>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <SectionHeader num="01" title="Performance Protocol" />
        ...
        {selectedWorkout ? (
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
              ...
              {entryForSelectedDate?.workout_done && entryForSelectedDate?.workout_day === selectedWorkout.name ? (
                ...
                <button
                  ...
                  disabled={saving}
                >
                  Mark Protocol Done for {formData.date}
                </button>
              )}
            </div>
          ...
          <div className="card text-center py-12 border-dashed border-border/40 bg-surface/20">
            <span className="text-[10px] text-muted mono uppercase tracking-[0.3em] font-black italic">Passive recovery state – No protocol assigned</span>
          </div>
        )}
      </section>

      {/* 02. BIOMETRIC METRICS */}
      <section>
        <SectionHeader num="02" title="Metric Injection" />
        <form onSubmit={handleSubmit} className="card p-8 space-y-10 border-border/40 bg-surface/50">
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-2 col-span-2 sm:col-span-1">
              <label className="text-[9px] mono uppercase text-muted font-black tracking-widest italic opacity-60 ml-1 block border-l-2 border-accent pl-2">Log Date</label>
              <input
                type="date"
          ...
          <div className="grid sm:grid-cols-2 gap-8 pt-10 border-t border-border/30">
            <div className="space-y-2">
              <label className="text-[9px] mono uppercase text-muted font-black tracking-widest italic opacity-60 ml-1 block border-l-2 border-green pl-2">Walk Volume (KM)</label>
              <input
                type="number"
                step="0.1"
                placeholder="0.0"
            ...
            <div className="space-y-2">
              <label className="text-[9px] mono uppercase text-muted font-black tracking-widest italic opacity-60 ml-1 block border-l-2 border-muted pl-2">Field Notes</label>
              <textarea
                className="input-field min-h-[58px] py-4 bg-surface2/40 font-mono text-[11px] leading-relaxed"
                placeholder="Hunger spikes, energy levels, hydration, sleep telemetry..."
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6">
            ...
            <button type="submit" className="btn btn-primary py-4 flex-1 text-[11px] mono uppercase font-black italic tracking-[0.2em] shadow-lg shadow-accent/10" disabled={saving}>
              {saving ? "Transmitting..." : "Commit Metrics to Engine"}
            </button>
          </div>
        </form>
      </section>

      {/* 03. ARCHIVE SNAPSHOT (COLLAPSIBLE) */}
      <section>
        <details className="group">
          <summary className="list-none flex items-center gap-4 cursor-pointer">
            <div className="flex items-center gap-4 flex-1">
              ...
              <h2 className="syne font-black uppercase text-sm tracking-widest italic text-muted group-hover:text-text transition-colors">Archive Snapshot</h2>
          ...
          <div className="mt-8 space-y-3 animate-in slide-in-from-top-4 duration-500">
            {entries.length === 0 && !loading && (
              <div className="text-center py-20 card border-dashed bg-surface/10">
                <span className="text-[10px] text-muted mono uppercase tracking-widest italic font-black">Historical records offline – No data detected</span>
              </div>
            )}
                ...
                <div className="flex items-center gap-8 w-full sm:w-auto">
                  ...
                  {entry.calories && (
                    <div className="hidden xs:flex flex-col border-r border-border/20 pr-8">
                      <span className="text-[9px] mono text-muted uppercase tracking-widest font-black opacity-50 mb-1 leading-none italic">Energy</span>
                  ...
                  <div className="flex items-center gap-2">
                    {entry.workout_done && (
                      <div className="badge badge-accent sm:px-3 sm:py-1 italic uppercase text-[9px] font-black tracking-widest">Protocol Done</div>
                    )}
                    {entry.body_fat && (
